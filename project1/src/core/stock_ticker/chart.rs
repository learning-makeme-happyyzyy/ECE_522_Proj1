...
pub fn print_closing_prices_and_dates(quotes: &Vec<Quote>, volatile_days: &Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    plot(&date_strs, &closing_prices, &high, &low, &positions)?;
    Ok(())
}

fn plot(dates: &[&str], closing_prices: &[f64], high: &[f64], low: &[f64], positions: &[usize]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("Closing Prices", ("sans-serif", 50).into_font())
        .margin(10)
